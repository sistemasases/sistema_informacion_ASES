...
const Info_general = (props) => {
  ...
  const handle_upload_estudiante = (e) => {
    ...
    axios({
      url:
        `${process.env.REACT_APP_API_URL}/usuario_rol/user_actualizacion/` +
        props.datos.id +
        "/",
      method: "PUT", // O "PATCH"
      data: formData,
  ...
  const handle_upload_info_extra = (e) => {
    ...
    axios({
      url:
        `${process.env.REACT_APP_API_URL}/usuario_rol/monitor_actualizacion/` +
        props.datos.id +
        "/",
      method: "POST",
      data: formData,
  ...
  return temporal ? (
    ...
    <Container className="container_informacion_general" xs={"12"} sm={"6"}>
      ...
      <Modal show={show} onHide={handleClose2}>
        ...
        <Modal.Body>
          {" "}
          {state.info_modal} el : {state.correo}
        </Modal.Body>
        <Modal.Footer>

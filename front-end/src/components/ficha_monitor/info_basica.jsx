...
const Info_basica_monitor = (props) => {
  ...
  const id_usuario_desencriptada = desencriptarInt(
    sessionStorage.getItem("id_usuario")
  );
  const id_sede_desencriptada = desencriptarInt(
    sessionStorage.getItem("sede_id")
  );
    ...
    axios({
      ...
      .then((respuesta) => {
        ...
        for (var i = 0; i < state.data_user["length"]; i++) {
          ...
          const url_axios =
            `${process.env.REACT_APP_API_URL}/usuario_rol/monitor/` +
            desencriptarInt(state.data_user[i]["id"]) +
            "/";
          axios({
            // Endpoint to send files
            url: url_axios,
  ...
  const handle_option_user = (e) => {
    ...
    console.log(e.id);
    const url_axios =
      `${process.env.REACT_APP_API_URL}/usuario_rol/monitor_info_extra/` +
      desencriptarInt(state.data_user[e.id]["id"]) +
      "/";
    axios({
      // Endpoint to send files
      url: url_axios,

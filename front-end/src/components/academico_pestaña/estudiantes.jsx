  ...
  const traer_cursos_del_estudiante = async (index) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/academico/traer_cursos_del_estudiante/` +
          index +
          "/",
        config
      );
            ...
            <Col className="contenido_fichas_academico2">
              {item.estudiantes
                .filter((item) => {
                  ...
                  return state.filtro.toLowerCase() === ""
                    ? item
                    : item.nombre
                        .toLowerCase()
          ...
          >
            <a
              href={`/calificador/${encodeURIComponent(
                ...
                item.curso_data.id_profesor
              )}/${encodeURIComponent(item.cod_materia)}/${encodeURIComponent(
                item.franja
              )}`}
              rel="noopener noreferrer"
              className="link_text_academico_hover4"
            >
              {item.curso_data.cod_materia}--{item.curso_data.franja} :{" "}
              {item.curso_data.nombre}
            </a>
          </Row>
        </Col>
      </Row>
      ...
      <Row>
        <Col className={open ? "fichas_academico3 open" : "fichas_academico3"}>
          <Row
            ...
            onClick={() => {
              setOpen(!open);
              traer_cursos_del_estudiante(item.id);
            }}
          >
            <Col className="link_text_academico1">
              <Row className="link_text_academico_hover3">
                <a>
                  <Link to={`/ficha_estudiante/${item.id}`}>
          ...
          <Row className="content_academico">
            <Col className="contenido_fichas_academico3">
              {state.cursos_estudiante.map((child, index) => (
                <Estudiantes key={index} item={child} />
              ))}
            </Col>
          </Row>
        </Col>

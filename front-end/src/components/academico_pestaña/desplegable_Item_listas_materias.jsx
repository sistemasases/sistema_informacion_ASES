...
const Desplegable_item_listas_materias = ({ item, franja }) => {
  ...
  const franjas_del_curso = async (index) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/academico/franja_curso/` +
          index +
          "/",
        config
      );
  ...
  const profesores_de_la_franja = async (index, index2) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/academico/profesores_del_curso/`,
  ...
  if (item.materias) {
    ...
    return (
      <>
        <Row>
          <Col
            className={open ? "fichas_academico2 open" : "fichas_academico2"}
          >
            <Row
              className="link_academico1"
              onClick={() => {
                setOpen(!open);
                franjas_del_curso(item.cod_materia);
              }}
            >
              <Col className="link_text_academico1">
                <Row className="link_text_academico_hover2">
            ...
            <Row className="content_academico">
              <Col className="contenido_fichas_academico3">
                {state.franjas_de_curso.map((child, index) => (
                  <Desplegable_item_listas_materias
                    key={index}
                    item={child}
                    franja={item.franja}
                  />
      ...
      <Row>
        <Col className={open ? "fichas_academico4 open" : "fichas_academico4"}>
          <Row className="link_text_academico_hover4">
            <a
              href={`/calificador/${encodeURIComponent(
                item.id
              )}/${encodeURIComponent(item.id_profesor)}/${encodeURIComponent(
                item.cod_materia
              )}/${encodeURIComponent(item.franja)}`}
              rel="noopener noreferrer"
              className="link_text_academico_hover4"
            >
              Grupo {item.franja} : {item.profesor_data.first_name}{" "}
              {item.profesor_data.last_name}
            </a>
          </Row>
    ...
    return (
      <Row>
        <Col className={open ? "fichas_academico4 open" : "fichas_academico4"}>
          ...
          <Row className="content_academico">
            <Col className="contenido_fichas_academico2" xs={4}></Col>
          ...
          <Row className="content_academico">
            <Col className="contenido_fichas_academico3">
              {state.alumnos_del_profesor.map((child, index) => (
            ...
            {item.notas ? (
              item.notas.length > 0 ? ( // Verificar si el array de notas no está vacío
                item.notas.map(
                  (
                    nota,
                    index // Si no está vacío, realizar el mapeo
                  ) => (
                    <Col key={index}>
                      ( {nota.nombre} : {nota.calificacion} )
                    </Col>
                  )
                )
              ) : (

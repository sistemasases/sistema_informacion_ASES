            ...
            if (!(!nuevo.email || nuevo.email === "")) {
              ...
              Create_User.user_rol(nuevo).then((res) => {
                if (res != null) {
                  ...
                  window.confirm(
                    "Error: El usuario ya se encuentra registrado o hay un error en los datos"
                  );
                }
              });
            } else {
      ...
      if (state.data["length"] > 0) {
        for (var i = 0; i < state.data["length"]; i++) {
          if (state.data[i].id === undefined) {
            axios
              .get(`${process.env.REACT_APP_API_URL}/usuario_rol/user/`, config)
              .then((res) => {
                for (var j = 0; j < res.data["length"]; j++) {
                  if (
                    state.data[i] &&
                    res.data[j] &&
                    res.data[j]["username"] === state.data[i]["username"]
                  ) {
                ...
                <tr>
                  ...
                  <td>
                    <Select
                      ...
                      defaultInputValue={e.nombre}
                      onChange={(c) => {
                        handleSelect(c.id, e.id);
                      }}
                    />
                  </td>
                  <td>
                    <Button variant="danger" onClick={() => handleClick(e)}>
                      Eliminar
                    </Button>
                  </td>
                </tr>

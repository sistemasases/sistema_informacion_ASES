...
const Asignaciones_component = (props) => {
  ...
  const isTabSelected = (section, index) =>
    index === selectedTabIndices[section];
    ...
    if (state.data_profesionales.length > state.opciones_profesionales.length) {
      for (var i = 0; i < state.data_profesionales.length; i++) {
        const dato = {
          ...
          label:
            state.data_profesionales[i]["username"] +
            " " +
            state.data_profesionales[i]["first_name"] +
            " " +
            state.data_profesionales[i]["last_name"],
  ...
  const [
    ...
    setIsLoading_practicantes_separados,
  ] = useState(true);
  useEffect(() => {
    if (
      state.separacion_practicantes.length > 0 &&
      isLoading_practicantes_separados
    ) {
  ...
  const [isLoading_monitores_separados, setIsLoading_monitores_separados] =
    useState(true);
  useEffect(() => {
    if (
      state.separacion_monitores.length > 0 &&
  ...
  const [isLoading_estudiantes_separados, setIsLoading_estudiantes_separados] =
    useState(true);
  useEffect(() => {
    if (
      state.separacion_estudiantes.length > 0 &&
  ...
  function practicante_seleccion(name) {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_URL}/usuario_rol/monitor/` + name + "/",
        formData,
        config
      )
      .then((response) => {
  ...
  function monitor_seleccion(name) {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_URL}/usuario_rol/estudiante_selected/` +
          name +
          "/",
        formData,
        config
      ...
      <Row className="row_listas">
        <Col xs={"12"} md={"4"}>
          <Row className="row_asignaciones_titulos">
            ...
            <Select
              options={state.opciones_profesionales}
              onChange={cambiar_dato_select}
            ></Select>
          </Row>
          <Row>
            {isLoading_practicantes_separados ? (
              <Col className="scroll_listas">
                ...
                <Scrollbars>
                  {state.data_practicantes
                    .filter((item) => {
                      return state.practicante_filtro.toLowerCase() === ""
                        ? item
                        : item.username
                            .toLowerCase()
                            .includes(state.practicante_filtro) ||
                            item.first_name
                              .toLowerCase()
                              .includes(state.practicante_filtro) ||
                            item.last_name
                    ...
                    .map((item, index) => (
                      <Col
                        className={
                          isTabSelected_practicante(item.username)
                            ? "asignaciones_hover_seleccionado"
                            : "asignaciones_hover_no_seleccionado"
                        }
                      ...
                      >
                        <Listas
                          ...
                          rol={rol}
                          profesional_seleccionado={
                            state.profesional_seleccionado
                          }
              ...
              <Col className="scroll_listas">
                <br></br>
                <Scrollbars>
                  {state.separacion_practicantes["0"]
                    .filter((item) => {
                      return state.practicante_filtro.toLowerCase() === ""
                        ? item
                        : item.username
                  ...
                  {state.separacion_practicantes["1"]
                    .filter((item) => {
                      return state.practicante_filtro.toLowerCase() === ""
                        ? item
                        : item.username
          ...
          <Row>
            {isLoading_monitores_separados ? (
              <Col className="scroll_listas">
                ...
                <Scrollbars>
                  {state.data_monitores
                    .filter((item) => {
                      return state.monitor_filtro.toLowerCase() === ""
                        ? item
                        : item.username
                            .toLowerCase()
                            .includes(state.monitor_filtro) ||
                            item.first_name
                              .toLowerCase()
                              .includes(state.monitor_filtro) ||
                            item.last_name
                    ...
                    .map((item, index) => (
                      <Col
                        className={
                          isTabSelected_monitor(item.username)
                            ? "asignaciones_hover_seleccionado"
                            : "asignaciones_hover_no_seleccionado"
                        }
          ...
          <Row>
            {isLoading_estudiantes_separados ? (
              <Col className="scroll_listas">
                ...
                <Scrollbars>
                  {state.data_estudiantes
                    .filter((item) => {
                      return state.estudiante_filtro.toLowerCase() === ""
                        ? item
                        : item.nombre
                            .toLowerCase()
                            .includes(state.estudiante_filtro) ||
                            item.apellido
                              .toLowerCase()
                              .includes(state.estudiante_filtro) ||
                            item.cod_univalle
              ...
              <Col className="scroll_listas">
                <Scrollbars>
                  {state.separacion_estudiantes["0"]
                    .filter((item) => {
                      return state.estudiante_filtro.toLowerCase() === ""
                        ? item
                        : item.nombre
                  ...
                  {state.separacion_estudiantes["1"]
                    .filter((item) => {
                      return (
                        state.estudiante_filtro.toLowerCase() === ""
                          ? item
                          : item.nombre
                              .toLowerCase()
                              .includes(state.estudiante_filtro),

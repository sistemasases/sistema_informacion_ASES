...
class info_estudiante_alertas_viewsets(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk):
        ...
        seguimientos_recientes = riesgo_individual.objects.filter(id_estudiante__in=estudiantes_ids).values('id_estudiante', 'riesgo_individual', 'riesgo_familiar', 'riesgo_academico', 'riesgo_economico', 'riesgo_vida_universitaria_ciudad','fecha')

        firma_tratamientos = firma_tratamiento_datos.objects.filter(id_estudiante__in=estudiantes_ids).values()
        ...
        for i in serializer_estudiantes.data:
            ...
            try:
                ...
                firma_tratamiento = next((s for s in firma_tratamientos if s['id_estudiante_id'] == estudiante_id), None)
                ...
                if seguimiento_reciente and firma_tratamiento:
                    riesgo = {
                ...
                elif seguimiento_reciente and not firma_tratamiento:
                    riesgo = {
                    'riesgo_individual': self.get_nivel_riesgo(seguimiento_reciente['riesgo_individual']),
                    'riesgo_familiar': self.get_nivel_riesgo(seguimiento_reciente['riesgo_familiar']),
                    'riesgo_academico': self.get_nivel_riesgo(seguimiento_reciente['riesgo_academico']),
                ...
                elif not seguimiento_reciente and not firma_tratamiento:
                    riesgo = {
